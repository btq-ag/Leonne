# Quantum Generalized Permutations
# A quantum-enhanced version of the Generalized Fisher-Yates algorithm
# capable of sampling permutations with quantum randomness

"""
quantumGeneralizedPermutations.py

Quantum-enhanced implementation of generalized permutation algorithms
that leverage quantum randomness for improved shuffling and permutation sampling.
...
def visualize_quantum_shuffle_animation(shuffle_result, filename=None, view_angle=(30, 45)):
    ...
    def animate(i):
        if i < len(quantum_states):
            ...
            # Redraw Bloch sphere - use the 2D arrays we created earlier            ax1.plot_surface(x, y, z, color='lightgray', alpha=0.1)
            ax1.plot([-1, 1], [0, 0], [0, 0], 'k-', alpha=0.5, linewidth=1)
            ax1.plot([0, 0], [-1, 1], [0, 0], 'k-', alpha=0.5, linewidth=1)
            ax1.plot([0, 0], [0, 0], [-1, 1], 'k-', alpha=0.5, linewidth=1)
            ax1.text(1.1, 0, 0, '|0⟩', fontsize=12)
